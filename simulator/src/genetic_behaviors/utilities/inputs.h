...
int get_inputs(int argc, char **argv, Inputs *inputs)
{
 ...
 inputs->nn_rec = 1;
  ...
  if (argc>1) {
        for(i=1;i<argc;i++){
          if (!strcmp(argv[i],"-h")){
                ...
                printf("Sintax: %s -x x_origen -y y_origen -a angle_robot -v x_destination -z y_destination -s sensor -n num_sensors -t theta_sensor -r range_sensor -p path_data -e environment -radio radio_robot -advance magnitud_advance -MaxAngle max_angle -steps number_steps -largest largest_value_sensors -noise add_noise -selection selection -vq quantize_sensors -size_vq vector_quantizer_size -pr_out random_outputs -nn_rec recurrent_nn\n",argv[0]);
                printf("examples:\n");
                printf("%s -x 1 -y 2.5 -a 3.1416 -v 2.0 -z 3.1 -s laser -n 4 -t -0.7075 -r 3.1416 -selection 3\n",argv[0]);
		printf("%s -x 0.28 -y 0.795 -a 0.0 -v 0.6525 -z 0.7875 -s laser -n 2 -t -0.2000 -r 0.4000 -radio 0.03 -advance 0.04 -MaxAngle 0.3 -steps 100 -largest 0.3 -noise 1\n",argv[0]);
  ...
  printf("%s -x %f -y %f -a %f -v %f -z %f -s %s -n %d -t %f -r %f -p %s -e %s -advance %f -MaxAngle %f -selection %d -largest %f -noise %d -omni %d -vq %d -size_vq %d -pr_out %d -out_file %s -nn_rec %d \n",
	...
	inputs->pr_out,inputs->output_file,inputs->nn_rec);
